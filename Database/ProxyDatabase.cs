...
namespace ChattingApp.Database
{
    public class ProxyDatabase
    {
        ...
        {
            SqlCommand command =
                new SqlCommand("insert into history values(@Sender,@Reciever,@Message,CURRENT_TIMESTAMP)",
                    myDB.getSqlConnection());
        ...
        {
            SqlCommand command = new SqlCommand("insert into users values(@name,@email,@password)",
                myDB.getSqlConnection());
        ...
        {
            SqlCommand command =
                new SqlCommand("SELECT portNum from users where email = @email and password = @password",
                    myDB.getSqlConnection());
            command.Parameters.AddWithValue("@email", email);
            command.Parameters.AddWithValue("@password", password);
        ...
        {
            SqlCommand command = new SqlCommand("SELECT * from users where email = @email and password = @password",
                myDB.getSqlConnection());
            command.Parameters.AddWithValue("@email", email);
            command.Parameters.AddWithValue("@password", password);
            SqlDataReader reader = await myDB.read(command);
